"""
Refresh Score Card data — fetch + score.

Pulls last 30 days of CLOSED tickets from Gorgias Unassigned/target agents,
samples up to 50 per agent, fetches messages, scores against the 12Q/100pt rubric.
Writes scorecard_data.json to CWD.

Env: GORGIAS_AUTH_TOKEN

Runtime: ~10-12 min (scans ~30K tickets + fetches ~1750 message bundles).
"""
...
def fetch_ticket_list(cutoff):
    """Paginate tickets by created_datetime desc, keep closed + target-agent + in window."""
    kept = []
    cursor = None
    batch = 0
    scanned = 0
...
def closed_date(t):
    d = t.get("closed") or t.get("created")
...
def main():
    ...
    print(f"Cutoff: {cutoff.isoformat()} ({WINDOW_DAYS}d)", flush=True)

    print("[1/3] Scanning ticket list...", flush=True)
    matched = fetch_ticket_list(cutoff)

    print("[2/3] Sampling + fetching messages...", flush=True)
    sampled = sample_per_agent(matched)
    ids = [t["id"] for t in sampled]
    print(f"  sample: {len(ids)} tickets across {len(set(t['__agent_name'] for t in sampled))} agents", flush=True)
    ...
    cutoff_7 = (datetime.now(timezone.utc) - timedelta(days=7)).date().isoformat()
    rows_7d = [r for r in ticket_rows if r["date"] and r["date"] >= cutoff_7]
    ...
    out = {
        ...
        "tickets": ticket_rows,
        "agents_30d": aggregate(ticket_rows),
        "agents_7d": aggregate(rows_7d) if rows_7d else [],
        "total_tickets_30d": len(ticket_rows),
        "total_tickets_7d": len(rows_7d),
    }
    ...
    print(f"  30d: {len(ticket_rows)} tickets · {len(out['agents_30d'])} agents")
    print(f"  7d:  {len(rows_7d)} tickets · {len(out['agents_7d'])} agents")
